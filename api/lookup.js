...
async function lookupProfile({ username, wallet }) {
  ...
  if (username) {
    ...
    tokens = await sbFetch(
      `bankr_launches?or=(x_username.eq.${u},x_username_fee.eq.${u})&select=*&order=launched_at.desc`,
      SUPABASE_ANON_KEY
    );
  }
  ...
  if (wallet && tokens.length === 0) {
    ...
    tokens = await sbFetch(
      `bankr_launches?or=(deployer_wallet.eq.${w},fee_recipient_wallet.eq.${w})&select=*&order=launched_at.desc`,
      SUPABASE_ANON_KEY
    );
  }
