...
const isAddr = (w) => /^0x[0-9a-f]{40}$/.test(w);
...
async function lookupProfile({ username, wallet }) {
  ...
  if (username) {
    ...
    tokens = await sbFetch(
      `bankr_launches?x_username=eq.${uEnc}&select=*&order=launched_at.desc`,
      SUPABASE_ANON_KEY
    );
    // Tokens where user is fee recipient (PleaseBro)
    pleaseBroTokens = await sbFetch(
      `bankr_launches?x_username_fee=eq.${uEnc}&select=*&order=launched_at.desc`,
      SUPABASE_ANON_KEY
    );
  ...
  for (const w of deployerWallets.slice(0, 3)) {
    ...
    const rows = await sbFetch(
      `bankr_claim_history?deployer_wallet=eq.${wl}&select=*`,
      SUPABASE_ANON_KEY
    );
    claimData.push(...rows);
  }
...
async function computeHolderStats(tokens) {
  ...
  for (const token of tokens) {
    ...
    try {
      holders = await sbFetch(
        `token_holders?token_address=eq.${addr}&select=holder_wallet,balance&order=balance.desc&limit=100`,
        SUPABASE_ANON_KEY
      );
    } catch { continue; }
    if (!holders.length) continue;
