...
async function saveFeeStructure(tokenAddress, wallet) {
  try {
    const r = await fetch(`https://api.bankr.bot/public/doppler/token-fees/${tokenAddress}?address=${wallet}&days=30`);
    if (!r.ok) return;
    const data = await r.json();
    const tok = (data.tokens || [])[0];
    if (!tok) return;
    await fetch(`${SUPABASE_URL}/rest/v1/bankr_launches?token_address=eq.${tokenAddress.toLowerCase()}`, {
      method: 'PATCH',
      headers: {
